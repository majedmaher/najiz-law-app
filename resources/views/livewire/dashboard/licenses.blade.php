<div class="mt-100 w-100">
    <div class="card p-20">
        ...
        <h3>{{__('main.licenses and credits')}}</h3>
        <h4>{{__('dashboard.page description', ['name'=> __('main.licenses and credits')])}}</h4>
        <div class="card m-10 p-50-100 md-p-20 shadow">            
            ...
            <form class="w-100 d-flex flex-column gap-50 mt-50" method="POST" wire:submit.prevent='submit'>
                <div class="d-grid grid-template-2 sm-grid-template-1 grid-gap-20">
                    <div class="grid-column-full">
                        <h4>{{__('dashboard.add item', ['name'=> __('dashboard.license')])}}</h4>
                    </div>
            ...
            @else
            <div class="save">
                <button wire:click='addBtn'>{{__('dashboard.add item', ['name' => __('dashboard.license')])}}</button>
            </div>
            @endif
            ...
            <form class="w-100 d-flex flex-column gap-50 mt-50" method="POST" wire:submit.prevent='update'>
                ...
                <div class="d-grid grid-template-2 sm-grid-template-1 grid-gap-20">
                    <div class="grid-column-full">
                        <h4>{{__('dashboard.update item', ['name'=> __('dashboard.license')])}}</h4>
                    </div>
                    <div class="form-group">
            ...
            <div class="mt-20" style="overflow-x:auto;">
                <table>
                  ...
                  @if(!empty($licenses) && $licenses->count())
                    @foreach($licenses as $key => $license)
                        <tr>
                            <td>{{ ++$key }}</td>
                            <td><img src="{{ asset($license->image) }}" alt="{{$license->title}}" width="50" height="50"></td>
                            <td>{{ $license->title }}</td>
                            <td>
                                <div class="actions">
                                    <button wire:click='editBtn({{$license->id}})' class="btn btn-info">Edit</button>
                                    <button wire:click='deleteItem({{$license->id}})' onclick="return confirm('{{__('dashboard.confirmation message')}}');" class="btn btn-danger">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @endforeach
                ...
                @if ($licenses->links()->paginator->hasPages())
                    <div class="mt-4 p-4 d-flex flex-justify-between">
                        {{ $licenses->onEachSide(1)->links('layouts.paginate') }}
                    </div>
                @endif
              </div>
        </div>
